...
| `code-auditor-10` | code-auditor | MEDIUM | [MEDIUM] `config_save_state` writes `backup.encryption_key` and the reCAPTCHA private key to the state file despite the comment saying it does not |  | - | - |
...
| `code-auditor-12` | code-auditor | MEDIUM | [MEDIUM] `deploy_run` reports success regardless of health-check results |  | related to conformance-auditor-18 (the probe target itself is unpublished) | - |
| `code-auditor-13` | code-auditor | LOW | [LOW] `_deploy_start_coturn` swallows every failure and then claims success |  | - | - |
| `code-auditor-14` | code-auditor | LOW | [LOW] Dendrite installs never get an admin account, and the manual fallback command names a Synapse container |  | - | - |
| `code-auditor-15` | code-auditor | LOW | [LOW] `--help` prints two lines of source code |  | duplicate of conformance-auditor-11 | - |
| `code-auditor-16` | code-auditor | LOW | [LOW] Dead local in `_deploy_start_services` |  | - | - |
...
| `code-auditor-35` | code-auditor | CRITICAL | [CRITICAL] The install directory is entirely root-owned but every container runs rootless as the matrix user |  | - | - |
| `code-auditor-36` | code-auditor | MEDIUM | [MEDIUM] `backup.retention_daily` / `retention_weekly` reach bash arithmetic unvalidated, giving command execution in the generated backup script |  | duplicate of security-auditor-6 | - |
| `code-auditor-37` | code-auditor | MEDIUM | [MEDIUM] The restore script cannot restore onto a clean host |  | related to code-auditor-40 (same restore path) | - |
| `code-auditor-38` | code-auditor | MEDIUM | [MEDIUM] A failed `pg_restore` is reported as "Database restored" |  | - | - |
| `code-auditor-39` | code-auditor | MEDIUM | [MEDIUM] `backup.sh` leaves an uncompressed full copy of the installation behind when it fails |  | - | - |
| `code-auditor-40` | code-auditor | MEDIUM | [MEDIUM] Backup and restore hardcode the `synapse` database name and user, ignoring the validated config keys |  | related to code-auditor-37 | - |
| `code-auditor-41` | code-auditor | LOW | [LOW] The retention policy advertises daily and weekly tiers but implements neither |  | duplicate of conformance-auditor-14 | - |
| `code-auditor-42` | code-auditor | LOW | [LOW] Backups are unencrypted by default and contain the signing key and every secret |  | duplicate of security-auditor-5. CONTRADICTION: code-auditor grades this LOW, security-auditor grades the same defect HIGH (F4). Not adjudicated here. | - |
| `code-auditor-43` | code-auditor | HIGH | [HIGH] `--upgrade` runs compose as root against a stack that was created rootless, and its Postgres major-version guard silently never fires |  | - | - |
| `code-auditor-44` | code-auditor | MEDIUM | [MEDIUM] The upgrade menu's "Reconfigure settings" option does nothing |  | - | - |
| `code-auditor-45` | code-auditor | MEDIUM | [MEDIUM] `--upgrade` bypasses `config_validate` entirely |  | - | - |
...
| `code-auditor-47` | code-auditor | MEDIUM | [MEDIUM] Media retention never purges anything: it authenticates with a token file that is never created |  | duplicate of conformance-auditor-12 | - |
| `code-auditor-48` | code-auditor | LOW | [LOW] Two computed values in the generated media-cleanup script are unused |  | - | - |
| `code-auditor-49` | code-auditor | LOW | [LOW] `media_retention_setup` depends on a directory `backup_setup` happens to create |  | - | - |
| `code-auditor-50` | code-auditor | MEDIUM | [MEDIUM] Coturn TLS can never be enabled: the gate tests a path that nothing creates |  | - | - |
...
| `code-auditor-56` | code-auditor | LOW | [LOW] `_bridge_setup_single` can call a previous bridge's registration function |  | - | - |
| `code-auditor-57` | code-auditor | LOW | [LOW] `bridges_setup` reports a count of requested bridges, not configured ones |  | - | - |
...
| `code-auditor-59` | code-auditor | HIGH | [HIGH] Rolling back after "Stop existing proxy" stops and disables it a second time instead of restoring it |  | - | - |
...
| `conformance-auditor-1` | conformance-auditor | HIGH | F-01 [HIGH] `advanced.install_dir` and `advanced.matrix_user` are read under the wrong key |  | - | - |
...
| `conformance-auditor-11` | conformance-auditor | MEDIUM | F-03 [MEDIUM] `--help` prints two lines of source code |  | duplicate of code-auditor-15 | - |
| `conformance-auditor-12` | conformance-auditor | HIGH | F-09 [HIGH] The media purge the timer runs can never authenticate, and targets an unpublished port |  | duplicate of code-auditor-47 | - |
| `conformance-auditor-13` | conformance-auditor | HIGH | F-10 [HIGH] `advanced.podman_compose_command` is documented, defaulted, and never read |  | related to conformance-auditor-15 (same ignored command setting) | - |
| `conformance-auditor-14` | conformance-auditor | HIGH | F-11 [HIGH] Backup retention has no weekly tier — the recovery window is 11 days, not 5 weeks |  | duplicate of code-auditor-41 | - |
| `conformance-auditor-15` | conformance-auditor | MEDIUM | F-12 [MEDIUM] `restore.sh` hard-codes `podman compose`, ignoring the detected tool |  | related to conformance-auditor-13 | - |
...
| `conformance-auditor-20` | conformance-auditor | HIGH | F-17 [HIGH] A Cloudflare token makes Caddy load a DNS module the pinned image does not contain |  | - | - |
...
| `coverage-analyst-9` | coverage-analyst | HIGH | A1 [HIGH] Bridge registration YAML: every assertion matches the key name, never the value |  | - | - |
| `coverage-analyst-10` | coverage-analyst | MEDIUM | A2 [MEDIUM] `bridge_compose_fragment` assertions cannot see an empty image |  | - | - |
| `coverage-analyst-11` | coverage-analyst | MEDIUM | A3 [MEDIUM] `bridge_description` / `bridge_requires_synapse` are checked for existence, never called |  | - | - |
...
| `compat-auditor-9` | compat-auditor | will-crash | F-09 [will-crash] The generated `matrix-stack.container` Quadlet has no `Image=`, so the Quadlet generator rejects it on every distro |  | duplicate of code-auditor-21 | - |
...
| `dependency-auditor-13` | dependency-auditor | LOW | DEP-013 - LOW - The digest drift gate turns a routine upstream event into a release failure |  | CONTRADICTION with ci-auditor-5: ci-auditor says the digest drift gate passes vacuously on zero images, dependency-auditor says it fails releases too readily. Not adjudicated here. | - |
...
| `security-auditor-2` | security-auditor | CRITICAL | F1 [CRITICAL] Config value reaches a `sed` script unescaped — root command execution (confidence 5) | accepted | fixed in working tree; 24 new tests across `test_compose_assembly.sh` and `test_config_validation.sh`, per-fix negative controls verified. `lib/19_compose.sh:189` now escapes `[&/\|]`; `monitoring.grafana_subdomain` and `dns.cloudflare_api_token` validated at `lib/04_config.sh:216-229`. Duplicate of code-auditor-23. | - |
...
| `security-auditor-5` | security-auditor | HIGH | F4 [HIGH] Backup archives carry the signing key and every config secret, unencrypted and world-readable (confidence 4) |  | duplicate of code-auditor-42. CONTRADICTION: graded HIGH here, LOW by code-auditor. Not adjudicated here. | - |
| `security-auditor-6` | security-auditor | MEDIUM | F5 [MEDIUM] `backup.retention_daily`/`retention_weekly` reach a bash arithmetic context — command execution in the daily timer (confidence 5) |  | duplicate of code-auditor-36 | - |
...
| ID | Source | Severity | Finding | Disposition | Evidence / Link | Approved by |
|---|---|---|---|---|---|---|
| `remed-1` | fix-compose | High | `_quadlet_generate_coturn` (`lib/20_quadlet.sh:99-130`) omits the `{{#TLS}}` certificate mount that `coturn.yml` carries, so TLS-enabled coturn gets no certs on the Quadlet path - the primary path. Same at the `podman run` fallback (`lib/21_deploy.sh:183-188`). | | | |
| `remed-2` | fix-rollback | High | `CONFIG[install_dir]` is never populated from TOML. The example config nests `install_dir` under `[advanced]`, so it parses as `advanced.install_dir` and nothing in `lib/` reads that key. Duplicate of `code-auditor-...` (`advanced.` prefix cluster) - cross-check before fixing. | | | |
...
| `remed-13` | work-dendrite | Medium | `lib/22_backup.sh:66,68,275` and `lib/26_upgrade.sh:83` hardcode `-U synapse … synapse`, ignoring `database.user` / `database.name`. | | | |
| `remed-14` | work-dendrite | Medium | `lib/23_media_retention.sh:33` calls `/_synapse/admin/v1/purge_media_cache`; Dendrite has no such endpoint. | | | |
| `remed-15` | work-dendrite | Medium | `lib/18_monitoring.sh:57` scrapes `/_synapse/metrics` on :9000; Dendrite serves `/metrics` on :8008 (`setup/base/base.go:140`). | | | |
| `remed-16` | work-dendrite | Medium | `lib/12_homeserver.sh:213` `homeserver_add_appservice` no-ops for Dendrite but logs success. Bridges silently do not register. | | | |
